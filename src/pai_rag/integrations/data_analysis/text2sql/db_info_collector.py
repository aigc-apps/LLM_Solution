...
class SchemaCollector(DBInfoCollector):
    ...
    def collect(self):
        ...
        logger.info("structured_db_description obtained.")
        # 保存为json文件
...
class ValueCollector(DBInfoCollector):
    ...
    def collect(self):
        ...
        unique_value_dict = self._get_unique_values(
            self._db_description_dict, self._max_col_num, self._max_val_num
        )
        logger.info("unique_value_dict obtained.")
        # # 保存为json文件
